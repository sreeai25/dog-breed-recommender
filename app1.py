...
def fetch_images_for_breed(breed_name, max_images=MAX_IMAGES_PER_BREED):
    images = []
    variants = normalize_variants(breed_name)
    exts = ["jpg", "jpeg", "png"]
    for folder in variants:
        for idx in range(1, max_images+1):
            for ext in exts:
                url = f"{GITHUB_REPO_RAW}/{quote(folder)}/{idx}.{ext}"
                try:
...
def display_images_with_fallback(breed_name):
    ...
    if images:
        ...
        folder = normalize_variants(breed_name)[0].replace(" ", "_")
...
# ---------------------
    ...
    # Show table of user scores for each trait
