...
if st.session_state.step < len(traits):
    ...
    st.progress((st.session_state.step+1)/total_traits)
    ...
    if cols_nav[0].button("⬅ Back", key=f"back_{trait}"):
        if st.session_state.step > 0:
            st.session_state.step -= 1
            st.experimental_rerun()
    if cols_nav[1].button("Skip", key=f"skip_{trait}"):
        st.session_state.skipped.add(trait)
        st.session_state.step += 1
        st.experimental_rerun()
    if cols_nav[2].button("Next ➡", key=f"next_{trait}"):
...
# ---------------------
    ...
    if st.session_state.answers:
        ...
        df_user = pd.DataFrame.from_dict(st.session_state.answers, orient='index', columns=["Your Importance"])
        st.table(df_user)
        ...
        choice = st.radio("What would you like to do next?", ["Exit", "Retake quiz", "Explain results"], key="post_result_choice")
        if st.button("Confirm", key="confirm_post_result"):
            if choice == "Exit":
